...
class WangModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # Eu nao retorno um lr_scheduler pq estou fazendo finetunning e, 
        # em teoria, nao preciso grandes alterações nos pesos
...
def main():
    ...
    pdb.set_trace()
    checkpoint = "/home/guilherme/text_style_transfer/checkpoints/wang_controllable_2019/last-v1.ckpt"
    state = torch.load(checkpoint, map_location="cpu")
    model = WangModel(BERT_CHECKPOINT)
    ...
    latent = model.autoencoder.sigmoid(z.last_hidden_state)
    latent = torch.sum(latent, dim=1)

    ys = torch.ones(1, 1).fill_(tokenizer.cls_token_id).long()
    for i in range(MAX_SEQUENCE_LENGTH - 1):
        x_hat = model.autoencoder.decoder(
            input_ids=ys, encoder_hidden_states=latent.unsqueeze(1)
        )
        probabilities = F.log_softmax(x_hat.logits, dim=-1)
    ...
    print(tokenizer.batch_decode(ys.tolist()))
